...
jobs:
  checks:
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/pr-checks.yml@v1.0.2
    secrets: inherit
```
...
jobs:
  label:
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/pr-label-check.yml@v1.0.2
```
...
jobs:
  tag:
    ...
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/auto-tag.yml@v1.0.2
    secrets: inherit
```
...
jobs:
  swiftformat-apply:
    ...
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/swiftformat-apply.yml@v1.0.2
```
...
- **`uses:` cannot contain expressions.** Composite action references inside the reusable workflows (e.g.
  `adesso-mobile-open-source/workflows-swift/actions/detect-platforms@v1.0.2`) are pinned to a static ref by necessity — this
...
This repository is tagged with semver tags (`1.2.3`) and a moving major tag (`v1.0.2`) that consuming packages
pin against in their `uses:` references, e.g. `...@v1.0.2`. Update the moving major tag when publishing
