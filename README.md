...
jobs:
  checks:
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/pr-checks.yml@v1.0.4
    secrets: inherit
```
...
jobs:
  label:
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/pr-label-check.yml@v1.0.4
```
...
jobs:
  tag:
    ...
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/auto-tag.yml@v1.0.4
    secrets: inherit
```
...
jobs:
  swiftformat-apply:
    ...
    uses: adesso-mobile-open-source/workflows-swift/.github/workflows/swiftformat-apply.yml@v1.0.4
```
...
- **`uses:` cannot contain expressions.** Composite action references inside the reusable workflows (e.g.
  `adesso-mobile-open-source/workflows-swift/actions/detect-platforms@v1.0.4`) are pinned to a static ref by necessity — this
  is a hard GitHub Actions restriction, not an oversight. Because the ref cannot be read from a variable at
  runtime, every occurrence is bumped in one step by a script (see [Versioning](#versioning-of-this-repository))
  rather than edited by hand.

## Versioning of this repository

This repository is tagged with semver tags (`1.2.3`) and a moving major tag (`v1.0.4`) that consuming packages
pin against in their `uses:` references, e.g. `...@v1.0.4`. Update the moving major tag when publishing
backwards-compatible releases.

### Bumping the version

The same version tag is pinned in many places (the reusable workflows in `.github/workflows/`, the examples in
`examples/caller-workflows/`, and this README). Since `uses:` refs cannot be templated, these cannot share a
single runtime variable. Instead, bump them all with one command:

```sh
./scripts/bump-version.sh v1.0.4
git commit -am 'Bump workflow version to v1.0.4'
```

`scripts/bump-version.sh` auto-detects the current version by scanning `.github/workflows/`, rewrites every
occurrence (in `@vX.Y.Z`, `ref: vX.Y.Z`, and README forms) to the new tag, and prints a summary. If the
workflows already contain more than one distinct tag (i.e. refs have drifted out of sync), it aborts and lists
the mismatches rather than guessing — reconcile them first, then re-run.
